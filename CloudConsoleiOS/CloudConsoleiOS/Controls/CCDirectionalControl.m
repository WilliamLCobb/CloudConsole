...
@implementation CCDirectionalControl
...
- (id)initWithFrame:(CGRect)frame BoundsImage:(NSString *)boundsImage StickImage:(NSString *)stickImage
{
    if (self = [super initWithFrame:frame]) {
        ...
        [self setStyle:CCDirectionalControlStyleJoystick];
    }
    return self;
}
...
- (id)initWithFrame:(CGRect)frame DPadImages:(NSArray <UIImage *> *)images;
{
    if (self = [super initWithFrame:frame]) {
        ...
        [self setStyle:CCDirectionalControlStyleDPad];
    }
    return self;
}
...
- (CGPoint)joyLocation
{
    if (self.style == CCDirectionalControlStyleDPad) {
...
- (CCDirectionalControlDirection)directionForTouch:(UITouch *)touch
{
    ...
    CCDirectionalControlDirection direction = 0;
    
    if (loc.x > (self.bounds.size.width + self.deadZone.width)/2) direction |= CCDirectionalControlDirectionRight;
