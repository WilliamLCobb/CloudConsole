...
- (BOOL)startScan {
    ...
    if ([self isIpAddressValid:self.localAddress] && (a.count == 4) && (b.count == 4)) {
        for (int i = 0; i<4; i++) {
            NSInteger and = [[a objectAtIndex:i] integerValue] & [[b objectAtIndex:i] integerValue];
...
- (void)pingAddress{
    ...
    NSMutableArray *a = [[self.baseAddress componentsSeparatedByString:@"."] mutableCopy];
    NSArray *b = [self.netMask componentsSeparatedByString:@"."];
    for (int i = 3; i>-1; i--) {
        if ([a[i] integerValue] + [b[i] integerValue] < 255) {
            a[i] = [NSString stringWithFormat:@"%ld", [a[i] integerValue] + 1];
            shouldContinue =YES;
            break;
        } else if ([a[i] integerValue] == 255) {
            a[i] = @"0";
        }
    }
    self.baseAddress = [a componentsJoinedByString:@"."];
    if (!shouldContinue) {
        [self.timer invalidate];
    } else {
        [SimplePingHelper ping:self.baseAddress target:self sel:@selector(pingResult:)];
    }
}
